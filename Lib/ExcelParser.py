...
class ExcelParser(object):
    ...
    def parseMain(self):
        ...
        NMACH = int(self.ws['A5'].value)
        self.rows.append([])
        for col in self.ws.iter_cols(min_row=5, max_col=NMACH + 1, max_row=5):
            for cell in col:
                self.rows[1].append(cell.value)

        NALPHA = int(self.ws['A7'].value)
        self.rows.append([])
        for col in self.ws.iter_cols(min_row=7, max_col=NALPHA + 1, max_row=7):
            for cell in col:
    ...
    def parsePanel(self, panNum):
        ...
        self.row_str.append(self.revColDel(self.rows[0], 11))
        self.row_str.append(self.revColDel(self.rows[1], 11))
        self.row_str.append(self.revColDel(self.rows[2], 11))
        self.row_str.append(self.revColDel(self.rows[3], 11))
        # self.row_str.append(self.colDel(self.rows[4],10))
        if self.NAP != 0:
            self.parseCamber()
        iter = 0
